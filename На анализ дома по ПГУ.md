...
15) Добавим радиус для функции установки башни, чтобы не промахиваться с их установкой
	1) в addPlaceholderTower после towerToSpawn.Parent = workspace добавляем эту функцию
	2) Дописываем переменную в функцию createRangeCircle `placeholder`, в addPlaceholderTower передаём false вместе с башней
	   ```lua
	   if placeholder then
		   p.Anchored = false
		   local weld = Instance.new("WeldConstrain")
		   weld.Part0 = p
...
```
	Соответственно, из основного кода убираем, что был до этого, т.е. переносим код
	3) Будет радиус не совсем красивый или удачный, поэтому допилим его
		```lua
		AddPlaceholderTower(name):
		CollisionGroup("Tower")
